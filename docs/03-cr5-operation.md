...
综合上位机的“机械臂与DH-AG95”页集中提供机械臂使能、负载、速度、报警、拖动
保护及末端夹爪控制；页面内容超出窗口时使用右侧滚动条。连接状态、关节角、末端
位姿和运行日志统一位于“连接、状态与日志”页，上下区域可拖动中间分隔条调整高度。
...
| 30003 | 运动指令 |
| 30005 | 当前 CR5 有线实时反馈（1440 字节帧） |
| 30004 | 仅兼容其他控制器；当前 CR5 不在此端口发送反馈字节 |

推荐设置 `DOBOT_FEEDBACK_PORT=30005`。未设置时驱动也默认使用 30005；只有确认
其他控制器确实从 30004 发送兼容帧时才显式改为 30004。连接成功日志应同时出现
29999、30003、30005。随后只读验证：
...
- 30005 默认反馈按完整 1440 字节帧重组，30004 可显式兼容，断线后可恢复；
